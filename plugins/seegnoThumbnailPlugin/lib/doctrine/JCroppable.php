...
class Doctrine_Template_JCroppable extends Doctrine_Template
{
  ...
  */
  ...
  {
    if (!isset($formOptions['with_delete']))
    {
      $formOptions['with_delete'] = true;
    }

    if (!isset($formOptions['template']))
    {
      $formOptions['template'] = '%file%<br />%input%<br />%delete% %delete_label%';
    }
    ...
    foreach ($this->_options['images'] as $fieldName) {
      ...
      $form->setWidget($fieldName, 
        new sfWidgetFormInputFileInputImageJCroppable(array(
          ...
          'with_delete' => $formOptions['with_delete'],
  ...
  private function getImageDir() {
    ...
    if (!empty($config[$tableName]['directory'])) {
      ...
      if (!empty($config[$tableName]['subdirectory'])) {
        $relativePath .= '/' . $this->getInvoker()->get($config[$tableName]['subdirectory']);
      }
    } else {
  ...
  public function getImageSrc($fieldName, $size = 'thumb') {
    ...
    $image = $this->getImageFromName($fieldName, $size);

    $fileSrc = '/' . $fileDir . '/' . $image;

    if (!$image || !file_exists($this->getImageDir() . DIRECTORY_SEPARATOR . $image))
    {
      if ($size == 'editable')
      {
        return false;
      ...
      $fileSrc = '/' . $fileDir . '/' . 'default_' . $size . '.jpg';
    }
  ...
  private function createCropForSize($fieldName, $size)
  {
    ...
    $ratio = isset($imageConfig['sizes'][$size]['ratio']) ? $imageConfig['sizes'][$size]['ratio'] : ($this->originalImages[$fieldName]->getWidth() / $this->editableImages[$fieldName]->getWidth());
    $height = (empty($imageConfig['ratio']) || empty($imageConfig['sizes'][$size]['ratio'])) ? null : $imageConfig['sizes'][$size]['width'] / ($imageConfig['sizes'][$size]['width'] / $imageConfig['sizes'][$size]['height']);

    $squarize = false;
    $info = getimagesize($this->originalImages[$fieldName]->getFilename());

    if (empty($height))
    {
      if (isset($imageConfig['sizes'][$size]['width']) && isset($imageConfig['sizes'][$size]['height']) && ($imageConfig['sizes'][$size]['width'] == $imageConfig['sizes'][$size]['height']))
      {
        if (($imageConfig['sizes'][$size]['width'] / $imageConfig['sizes'][$size]['height']) == ($info[0] / $info[1]))
        {
          $height = $imageConfig['sizes'][$size]['width'];
        }
        else
        {
          $squarize = true;
      ...
      {
        $height = ($imageConfig['sizes'][$size]['width'] * $info[1]) / $info[0];
      }
    }

    if ($squarize)
    {
      $width = ($info[1] > $info[0]) ? $info[0] : $info[1];
      $marginLeft = ($info[0] / 2)-($width / 2);
      $marginBottom = ($info[1] / 2)-($width / 2);
      ...
      {
        $dims['w'] = (int) (($width + $marginLeft));
        $dims['h'] = (int) (($width + $marginBottom));
      }
    }
    else
    {
    ...
    $origCrop = $this->originalImages[$fieldName]->crop($dims['x'], $dims['y'], $dims['w'], $dims['h']);

    $finalCrop = $origCrop->resize($imageConfig['sizes'][$size]['width'], $height);
